...
func (ts *ExternalTestSuite) tokenExchange(provider, subjectToken string) *httptest.ResponseRecorder {
	return ts.tokenExchangeWithType(provider, subjectToken, accessTokenSubjectTokenType)
}

func (ts *ExternalTestSuite) tokenExchangeWithType(provider, subjectToken, subjectTokenType string) *httptest.ResponseRecorder {
...
func (ts *ExternalTestSuite) TestTokenExchangeSignsInExistingIdentity() {
	...
	w := ts.tokenExchange("facebook", "valid_access_token")
...
func (ts *ExternalTestSuite) TestTokenExchangeProviderIsCaseInsensitive() {
	user := ts.createFacebookIdentity("facebookTestId", "facebook@example.com")

	server := FacebookAccessTokenSetup(ts, ts.Config.External.Facebook.ClientID[0], true, "USER", "facebookTestId")
	defer server.Close()

	// "Facebook" must resolve to the stored lower-cased "facebook" identity.
	w := ts.tokenExchange("Facebook", "valid_access_token")
	ts.Require().Equal(http.StatusOK, w.Code, w.Body.String())
...
func (ts *ExternalTestSuite) TestTokenExchangeRejectsTokenFromAnotherApp() {
	...
	w := ts.tokenExchange("facebook", "token_for_another_app")
	ts.Require().Equal(http.StatusBadRequest, w.Code)
}
...
func (ts *ExternalTestSuite) TestTokenExchangeRejectsInvalidToken() {
	...
	w := ts.tokenExchange("facebook", "invalid_token")
	ts.Require().Equal(http.StatusBadRequest, w.Code)
}
...
func (ts *ExternalTestSuite) TestTokenExchangeRejectsNonUserToken() {
	...
	w := ts.tokenExchange("facebook", "page_token")
	ts.Require().Equal(http.StatusBadRequest, w.Code)
}

func (ts *ExternalTestSuite) TestTokenExchangeMissingSubjectToken() {
	w := ts.tokenExchange("facebook", "")
	ts.Require().Equal(http.StatusBadRequest, w.Code)
}

func (ts *ExternalTestSuite) TestTokenExchangeMissingProvider() {
	w := ts.tokenExchange("", "some_token")
	ts.Require().Equal(http.StatusBadRequest, w.Code)
}

func (ts *ExternalTestSuite) TestTokenExchangeUnsupportedSubjectTokenType() {
	w := ts.tokenExchangeWithType("facebook", "some_token", "urn:ietf:params:oauth:token-type:id_token")
	ts.Require().Equal(http.StatusBadRequest, w.Code)
}

func (ts *ExternalTestSuite) TestTokenExchangeUnsupportedProvider() {
	...
	w := ts.tokenExchange("google", "some_google_token")
	ts.Require().Equal(http.StatusBadRequest, w.Code)
}
...
func (ts *ExternalTestSuite) TestTokenExchangeProviderDisabled() {
	...
	w := ts.tokenExchange("facebook", "some_token")
	ts.Require().Equal(http.StatusBadRequest, w.Code)
}
...
func (ts *ExternalTestSuite) tokenExchangeLink(provider, subjectToken, authToken string) *httptest.ResponseRecorder {
	var buffer bytes.Buffer
	ts.Require().NoError(json.NewEncoder(&buffer).Encode(map[string]interface{}{
		"provider":           provider,
		"subject_token":      subjectToken,
...
func (ts *ExternalTestSuite) TestTokenExchangeLinkIdentity() {
	...
	w := ts.tokenExchangeLink("facebook", "valid_access_token", authToken)
	ts.Require().Equal(http.StatusOK, w.Code, w.Body.String())
...
func (ts *ExternalTestSuite) TestTokenExchangeLinkIdentityRequiresAuth() {
	...
	w := ts.tokenExchangeLink("facebook", "valid_access_token", "")
	ts.Require().Equal(http.StatusBadRequest, w.Code)
}
